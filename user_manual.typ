...
#block(
  ...
  - *LaTeXSnipper 桌面端：* 只有截图 OCR 需要本机运行并开启 Office 插件功能；普通公式插入、加载、更新、转换、编号和引用均由插件本地完成
]
...
#block(
  ...
  多选转换和多选格式化所选会先收集稳定快照（公式 ID、起始位置、渲染类型和必要元数据），不跨批持有 live COM object。转换和涉及替换的格式化按文档位置倒序分批执行，每批 5 个公式，每批更新一次状态窗格；Word 每批使用短 undo record。长公式或大量公式时，主要耗时仍来自 `r`（MathJax 渲染/图片生成）和 `ole/xml/png`（Office COM 插入与对象替换）。建议长文档批量清理时分段执行并保存文档。
